...
def run_inference(args):
    ...
    # ================= XLA 设备情况与 metrics 初始化 =================
    is_xla = "xla" in str(device)

    print(f"[Device] Using device: {device}")
    if is_xla and xm is not None:
        try:
            print(f"[XLA] Available devices: {xm.get_xla_supported_devices()}")
        except Exception as e:
            print(f"[XLA] get_xla_supported_devices() failed: {e}")

        if met is not None:
            try:
                met.clear_metrics()
                print("[XLA] Cleared XLA metrics before inference.")
            except Exception as e:
                print(f"[XLA] clear_metrics() failed: {e}")
    ...
    # ================= 后台心跳监控线程 =================
    monitor_interval = 60.0  # seconds
    stop_event = threading.Event()

    def _monitor():
        while not stop_event.wait(monitor_interval):
            elapsed = time.time() - start_time
            print(f"[MONITOR] Elapsed: {elapsed:.1f} s (inference still running...)")
            if is_xla and met is not None:
                try:
                    print("[MONITOR][XLA] metrics_report():")
                    print(met.metrics_report())
                except Exception as e:
                    print(f"[MONITOR][XLA] metrics_report() failed: {e}")
    ...
    # Ensure XLA ops are executed for accurate timing.
    if is_xla and xm is not None:
        try:
            xm.mark_step()
            xm.wait_device_ops()
        except Exception as e:
            print(f"[XLA] xm.wait_device_ops() failed: {e}")

    total_time = time.time() - start_time
    per_frame_time = total_time / len(views)
    print(
        f"Inference completed in {total_time:.2f} seconds "
        f"(average {per_frame_time:.2f} s per frame for {len(views)} views)."
    )

    # ================= 打印 XLA metrics_report =================
    if is_xla and met is not None:
        print("\n================ XLA metrics_report() ================\n")
        try:
            print(met.metrics_report())
        except Exception as e:
            print(f"[XLA] metrics_report() failed: {e}")
        print("\n======================================================\n")
