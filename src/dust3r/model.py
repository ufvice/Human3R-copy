...
class ARCroco3DStereo(CroCoNet):
    ...
    def __init__(self, config: ARCroco3DStereoConfig):
        ...
        if config.output_mode == "naive":
            self.fourier_camera = FourierPositionEncoding(
            ...
            self.cross_queries_x = nn.Parameter(
                torch.zeros(self.bb_token_res, context_dim)
            )
            torch.nn.init.normal_(self.cross_queries_x, std=0.2)
            self.cross_queries_y = nn.Parameter(
                torch.zeros(self.bb_token_res, context_dim)
            )
            torch.nn.init.normal_(self.cross_queries_y, std=0.2)
            self.cross_values_x = nn.Parameter(
                torch.zeros(self.bb_token_res, context_dim)
            )
    ...
    def set_freeze(self, freeze):  # this is for use by downstream models
        ...
        if freeze == "encoder_and_decoder_and_head":
            fix_all_params(
                to_be_frozen["encoder_and_decoder_and_head"]
            )  # will not be updated
    ...
    def _encode_views_mhmr(self, views, img_mask=None, ray_mask=None):
        ...
        if selected_imgs_mhmr.size(0) > 0:
            ...
            ]  # image[bs, 3, h, w] -> image feature [bs, h_nb_patches * w_nb_patches, D]
        else:
            raise NotImplementedError
        ...
        mhmr_full_out = [
            torch.zeros(
                len(views) * batch_size,
                *mhmr_img_out[0].shape[1:],
                device=mhmr_img_out[0].device,
            )
    ...
    def _decoder(
        ...
        if self.pose_head_flag:
            ...
            if f_smpl is not None:
                ...
                pos_img = torch.cat(
                    [pos_pose, pos_img], dim=1
                )  # used for naive CUT3R+MHMR
    ...
    def embedd_camera(self, K, n_patch):
        ...
        points = (
            points * self.bb_patch_size + self.bb_patch_size // 2
    ...
    def smpl_tokenizer_mhmr(self, feat, pos, views, inference=False):
        feat = torch.stack(
            [f.detach() for f in feat], dim=0
        )  # (num_view, bs, 576, 1024)
        ...
        if self.msk_head_flag:
            msks = self.downstream_head.segment(
                feat.detach()
            )  # (num_view * bs, 576, 14*14)
        ...
        if self.output_mode == "naive":
            ...
            feat_K = self.embedd_camera(
                K, [n_patch, n_patch]
            )  # Embed viewing directions. [num_view * bs,h,w,99]
        ...
        if inference:
            ...
            loc_full_out = torch.zeros(
                num_view * batch_size, max_humans, loc.shape[1], device=loc.device
            )
        ...
        if self.output_mode == "naive":
            ...
            head_token = torch.cat(
                ...
                dim=-1,
            )  # training: [bs, 10, 1454]; inference: [nhv, 1454]
        ...
        if inference:
            ...
            for i in range(num_view * batch_size):
                if self.output_mode == "naive":
                    smpl_query = self.transformer(
                        ...
                        mask=None,
                    )  # train:[bs, 10, 1024]), inference:[1, nhv, 1024]
                else:
                    smpl_query = head_token[img_id == i].unsqueeze(
                        0
                    )  # use mhmr vit token
                smpl_query_list.append(smpl_query)
                smpl_pos = (
                    torch.zeros(*smpl_query.shape[:2], 2)
                    .to(smpl_query.device)
                    .to(pos[0].dtype)
                )
            ...
            if self.output_mode == "naive":
                smpl_query = self.transformer(
                    head_token, context=feat_all, mask=smpl_mask.type(torch.float32)
                )  # [bs, 10, 1024])
            else:
    ...
    def smpl_tokenizer_cut3r(self, feat, pos, views, loc, inference=False):
        ...
        if inference:
            ...
            full_pos = torch.zeros(
                ...
                max_humans,
                pos_central.shape[1],
                device=pos_central.device,
                dtype=pos_central.dtype,
            )
            full_out[smpl_mask] += feat_central
        ...
        if inference:
            smpl_query_list = [
                smpl_query[img_id == i].unsqueeze(0)
                for i in range(num_view * batch_size)
            ]
            smpl_pos_list = [
                pos_central[img_id == i].unsqueeze(0)
                for i in range(num_view * batch_size)
            ]
            smpl_uv_list = [
    ...
    def token_fuse(self, tk_mhmr, tk_cut3r, inference):
        if inference:
            ...
            tk_mhmr = torch.stack(
                [t.detach() for t in tk_mhmr], dim=0
            )  # (num_view, bs, 10, 1024)
            tk_cut3r = torch.stack(
                [t.detach() for t in tk_cut3r], dim=0
            )  # (num_view, bs, 10, 1024)
            num_view, batch_size = tk_mhmr.shape[:2]

            tk_mhmr = tk_mhmr.view(-1, *tk_mhmr.shape[2:])  # (num_view * bs, 10, 1024)
            tk_cut3r = tk_cut3r.view(
                -1, *tk_cut3r.shape[2:]
            )  # (num_view * bs, 10, 1024)
    ...
    def _forward_impl(self, views, ret_state=False, inference=False):
        ...
        for i in range(len(views)):
            ...
            out_pose_feat_i = dec[-1][
                :, 0:1
            ]  # After Cross-Attention, refined pose feat: [b, 1, 768]
            ...
            if self.msk_head_flag:
                res["msk"] = msks[i]
            ress.append({**res, "smpl_scores": scores[i], "smpl_loc": smpl_loc[i]})
            img_mask = views[i]["img_mask"]
            update = views[i].get("update", None)
    ...
    def forward(self, views, ret_state=False, inference=False):
        if self.output_mode == "naive":
            if ret_state:
                ress, views, state_args = self._forward_impl_naive(
                    views, ret_state=ret_state, inference=inference
                )
    ...
    # -------------------------------------------------------------------------
    # [Fix] Helper function to avoid CPU syncs in unpad_uv
    # -------------------------------------------------------------------------
    def _unpad_uv_tensor_safe(self, uv, original_size, target_height, target_width):
        """
        A TPU-friendly version of unpad_uv that assumes all inputs are Tensors
        and strictly avoids .item(), float(), or int() conversions.
        """
        dtype = uv.dtype

        # Ensure scalar size inputs are float tensors without host sync
        if not target_height.is_floating_point():
            target_height = target_height.to(dtype)
        if not target_width.is_floating_point():
            target_width = target_width.to(dtype)

        if not torch.is_tensor(original_size):
            original_size = torch.tensor(original_size, device=uv.device, dtype=dtype)
        else:
            original_size = original_size.to(device=uv.device, dtype=dtype)

        max_target = torch.maximum(target_height, target_width)
        scale_factor = max_target / original_size
        uv_scaled = uv * scale_factor

        pad_left = torch.floor((max_target - target_width) / 2.0)
        pad_top = torch.floor((max_target - target_height) / 2.0)

        offset = torch.stack([pad_left, pad_top], dim=-1)
        uv_transformed = uv_scaled - offset

        uv_transformed[..., 0] = torch.clamp(
            uv_transformed[..., 0], min=0.0, max=(target_width - 1.0)
        )
        uv_transformed[..., 1] = torch.clamp(
            uv_transformed[..., 1], min=0.0, max=(target_height - 1.0)
        )
    ...
    def forward_recurrent_lighter(
        ...
        for i, _view in enumerate(views):
            ...
            # --- Encoding Image (Dust3r) ---
            imgs = view["img"].unsqueeze(0)
            if "true_shape" in view:
                shapes = view["true_shape"].unsqueeze(0)
            else:
                s = torch.tensor(view["img"].shape[-2:], device=device)
                shapes = s.unsqueeze(0).repeat(batch_size, 1).unsqueeze(0)

            imgs = imgs.view(-1, *imgs.shape[2:])
            shapes_flat = shapes.view(-1, 2)

            img_out, img_pos, _ = self._encode_image(imgs, shapes_flat)
            feat_i = img_out[-1]
            pos_i = img_pos
            shape = shapes_flat
            ...
            mhmr_img_out = [self.backbone(imgs_mhmr)]
            ...
            scores_flat = scores.view(B_scores, -1)
            best_idx = torch.argmax(scores_flat, dim=1)

            img_id = torch.arange(B_scores, device=device)
            ...
            loc_cut3r = self._unpad_uv_tensor_safe(
                loc,
                self.mhmr_img_res,
                th,
                tw,
            ...
            smpl_uv_cut3r = (loc_cut3r // patch_size).long()
            w_id_cut3r, h_id_cut3r = smpl_uv_cut3r.T

            h_id_cut3r = h_id_cut3r.clamp(0, n_patch_cut3r_h - 1)
            w_id_cut3r = w_id_cut3r.clamp(0, n_patch_cut3r_w - 1)
    ...
    def forward_recurrent_lighter_naive(
        self, views, device, ret_state=False, use_ttt3r=False
    ):
        pass
