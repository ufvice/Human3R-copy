...
class ARCroco3DStereo(CroCoNet):
    ...
    def forward_recurrent_lighter(
        ...
        for i, _view in enumerate(views):
            ...
            scores = self.downstream_head.detect_mhmr(
                feat_mhmr_i
            )  # (num_view * bs, h_patches*w_patches, 1)
            scores = rearrange(
                scores, "b (nh nw) c -> b c nh nw", nh=n_patch_mhmr, nw=n_patch_mhmr
            )  # [num_view * bs, h_nb_patches * w_nb_patches, 1] -> [num_view * bs, 1, h, w]
            if self.msk_head_flag:
                msks = self.downstream_head.segment(feat_mhmr_i)  # low-res mask
                msks = rearrange(
                    msks, "b (nh nw) c -> b c nh nw", nh=n_patch_mhmr, nw=n_patch_mhmr
                )
                msks = F.pixel_shuffle(
                    msks, self.bb_patch_size
                )  # (num_view * bs, 1, h, w)
            ...
            # fuse CUT3R and MHMR smpl tokens
            fused_tk = torch.cat(
                [smpl_tk_mhmr, smpl_tk_cut3r], dim=-1
            )  # (1, nvh, 2048)
            ...
            if use_ttt3r and i != 0 and not reset_mask:
                ...
                update_mask_state = (
                    update_mask * torch.sigmoid(cross_attn_states)[..., None]
                )
            else:
    ...
    def forward_recurrent_lighter_naive(
        ...
        for i, _view in enumerate(views):
            ...
            feat_K = self.embedd_camera(
                K, [n_patch_mhmr, n_patch_mhmr]
            )  # Embed viewing directions. [num_view * bs,h,w,99]
            feat_K_central = feat_K[img_id, h_id, w_id]  # (nvh, 99)
            feat_central_mhmr = torch.cat(
                [feat_central_mhmr, feat_K_central], 1
            )  # feature + camera embedding for heads only to query tokens [nhv, 1123]
            feat_all = torch.cat([feat_mhmr_i, feat_K], -1).permute(
                0, 3, 1, 2
            )  # feature + camera embedding for full image for the cross-attention only. [bs,1123,nh,nw]
            ...
            smpl_tk_mhmr = self.transformer(
                feat_central_mhmr.unsqueeze(0), context=feat_all, mask=None
            )  # inference:[1, nhv, 1024]
